...
**Your Answers here:**
1. This is done in the 'mytickers' part of the code in dataSet1.R. mytickers = c("SPY", "AAPL")
2. Cumulative: 596.5%; Average daily: 0.148%; SD: 2.389
3. Yahoo. Cumulative: 291.2%; Average daily: 0.072%; SD: 3.019
...
**Your Answers here:**
1. 2001-01-01 was a bank holiday.The last day of trading before was the 2000-12-29, and the next day of trading 2001-01-02. I have adjusted the data to start on the 2000-12-29, which makes the data start one day earlier. To get the new exercise, the code must be changed, saved and then relinked to dataSet1.R
2.See below for data on Microsoft and Apple from the start of 2016 to present.
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
portfolio = apply(StockReturns,1,mean)
names(portfolio) <- rownames(StockReturns)
pnl_plot(portfolio)
...
**Your Answers here:**
1. It gives the number of lines in the data (CSV file).
2. The addition doesn't work. The two are in different formats - myData is a CSV file and StockReturns is a matrix.
<br>
<br>
...
```{r eval = TRUE, echo=TRUE, comment=NA, warning=FALSE, message=FALSE,results='asis'}
write.csv(StockReturns[1:30,c("SPY","AAPL")], file = "thirtydays.csv", row.names = TRUE, col.names = TRUE) 
```

```{r eval = TRUE, echo=TRUE, comment=NA, warning=FALSE, message=FALSE,results='asis'}
myData2 <- read.csv(file = "thirtydays.csv", header = TRUE, sep=";")
```
